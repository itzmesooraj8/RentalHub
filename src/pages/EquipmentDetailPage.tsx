        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div className="space-y-3">
            <div className="relative aspect-16/10 rounded-3xl overflow-hidden bg-[#111111] border border-[#1F1F1F] shadow-2xl">
              ...
              <button
                ...
                className={`absolute top-4 right-4 p-2.5 rounded-full backdrop-blur-md transition cursor-pointer ${
                  isFav ? 'bg-[#F27D26] text-black' : 'bg-[#0A0A0A]/70 text-white hover:bg-[#0A0A0A]'
                }`}
              >
              ...
              <div className="flex gap-3">
                {equipment.images.map((img, idx) => (
                  <button
                    ...
                    className={`w-20 h-20 rounded-2xl overflow-hidden border transition cursor-pointer ${
          ...
          <div className="bg-[#111111] rounded-3xl p-6 sm:p-8 border border-[#1F1F1F] shadow-xl space-y-4 font-mono">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {Object.entries(equipment.specs).map(([key, val]) => (
                <div key={key} className="p-3 bg-[#1A1A1A] rounded-2xl border border-[#222222] flex justify-between items-center text-xs">
          ...
          <div className="bg-[#111111] rounded-3xl p-6 sm:p-8 border border-[#1F1F1F] shadow-xl space-y-4 font-mono">
            ...
            {equipmentReviews.length === 0 ? (
              <p className="text-xs text-[#666666] italic">No verified reviews submitted yet for this machine.</p>
            ) : (
              <div className="space-y-4">
                {equipmentReviews.map((rev) => (
                  <div key={rev.id} className="p-4 rounded-2xl bg-[#1A1A1A] border border-[#222222] space-y-2 text-xs">
