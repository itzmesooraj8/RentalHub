...
export const OwnerEquipmentPage = ({
  ...
  const ownerItems = currentUser ? equipmentList.filter((eq) => eq.ownerId === currentUser.id || true) : equipmentList;
  const filteredItems = ownerItems.filter((item) => {
    const q = searchQuery.toLowerCase().trim();
    const matchesSearch = !q || item.title.toLowerCase().includes(q) || item.category.toLowerCase().includes(q) || item.description.toLowerCase().includes(q);
  ...
  >
    ...
    className={`p-2 rounded-lg transition ${viewMode === "table" ? "bg-[#F27D26] text-black" : "text-[#888888] hover:text-white"}`}
    title="Table View"
  >
    ...
    className={`p-2 rounded-lg transition ${viewMode === "grid" ? "bg-[#F27D26] text-black" : "text-[#888888] hover:text-white"}`}
    title="Grid View"
  >
    ...
    /* Equipment Table / Grid */
      ...
      {filteredItems.length === 0 ? <div className="bg-[#111111] rounded-3xl p-12 text-center border border-[#1F1F1F] max-w-md mx-auto space-y-3 font-mono">
  ...
  >
        ...
        </div> : viewMode === "table" ? <div className="bg-[#111111] rounded-3xl border border-[#1F1F1F] overflow-hidden shadow-xl">
          <div className="overflow-x-auto">
            <table className="w-full text-left text-xs font-mono">
              ...
              <tbody className="divide-y divide-[#1F1F1F]">
                {filteredItems.map((item) => <tr key={item.id} className="hover:bg-[#141414] transition">
                    <td className="p-4">
                    ...
                    <td className="p-4 text-[#AAAAAA]">
                      <div className="flex items-center gap-2">
                        <div className="w-16 h-2 rounded-full bg-[#222222] overflow-hidden">
                          <div className="h-full bg-[#F27D26]" style={{ width: "75%" }} />
                        </div>
  ...
  >
          ...
          {filteredItems.map((item) => <div key={item.id} className="bg-[#111111] rounded-3xl p-5 border border-[#1F1F1F] shadow-xl space-y-4 flex flex-col justify-between">
