...
export const BrowsePage: React.FC<BrowsePageProps> = ({
  ...
  const paramView = (searchParams.get('view') as 'grid' | 'map') || 'grid';
  ...
  const [viewMode, setViewMode] = useState<'grid' | 'map'>(paramView);

  // Keep state in sync if URL search params change externally (e.g. clicking footer link)
  useEffect(() => {
    const newCat = searchParams.get('category') || 'All';
    const newSearch = searchParams.get('search') || '';
    const newView = (searchParams.get('view') as 'grid' | 'map') || 'grid';
  ...
  // Debounce search input for performance & update URL
  useEffect(() => {
  ...
  const updateUrlParams = (cat: string, q: string, v: 'grid' | 'map', price: number) => {
  ...
  const handleViewModeChange = (newView: 'grid' | 'map') => {
    setViewMode(newView);
    updateUrlParams(category, searchInput, newView, maxRate);
  };

  const handleMaxRateChange = (newRate: number) => {
    setMaxRate(newRate);
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 text-white font-mono">
      ...
      <div className="flex items-center justify-between text-xs font-mono text-[#888888] px-1">
        ...
        {(searchInput || category !== 'All' || maxRate < 1200 || viewMode !== 'grid') && (
          <button
