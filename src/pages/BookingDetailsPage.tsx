...
export const BookingDetailsPage: React.FC<BookingDetailsPageProps> = ({
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 font-mono text-white">
      {/* Top Navigation */}
            ...
            return (
              <div
                ...
                className={`p-3 rounded-2xl border text-center transition-all ${
                  isCurrent
                    ? 'bg-[#F27D26]/10 border-[#F27D26] text-white shadow-lg'
                    : isDone
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="bg-[#111111] rounded-3xl p-6 border border-[#1F1F1F] shadow-xl space-y-4">
            ...
            {booking.condition?.pickupPhotos?.length || booking.condition?.returnPhotos?.length ? (
              <div className="space-y-3 text-xs">
                {booking.condition?.pickupNotes && (
                  <p className="text-[#888888] bg-[#1A1A1A] p-3 rounded-2xl border border-[#222222]">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div>
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-1">
                      {(booking.beforePhotos || booking.condition?.pickupPhotos || []).map((img, i) => (
                        <img key={i} src={img} alt="Pickup" className="w-16 h-16 rounded-xl object-cover border border-[#333]" />
                      ))}
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="flex gap-2 overflow-x-auto pb-1">
                      {(booking.afterPhotos || booking.condition?.returnPhotos || []).map((img, i) => (
                        <img key={i} src={img} alt="Return" className="w-16 h-16 rounded-xl object-cover border border-[#333]" />
                      ))}
                    </div>
                  </div>
                </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-[#111111] rounded-3xl p-6 border border-[#1F1F1F] shadow-xl space-y-3 font-mono text-xs">
            ...
            {(booking.status === 'pending' || booking.status === 'confirmed' || booking.status === 'locked') && onCancelBooking && (
              <button
                onClick={() => {
      ...
      {isConditionModalOpen && onSubmitConditionReport && (
        <DamageReportModal
          booking={booking}
