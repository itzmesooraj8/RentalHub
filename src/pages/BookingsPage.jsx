...
export const BookingsPage = ({
  ...
  const getStatusBadge = (status, hasDispute) => {
    if (hasDispute || status === "disputed") {
      return <span className="px-2.5 py-1 rounded-full bg-rose-500/10 text-rose-400 border border-rose-500/30 text-[10px] font-mono font-bold uppercase tracking-wider flex items-center gap-1">
    ...
    switch (status) {
      case "pending":
        return <span className="px-2.5 py-1 rounded-full bg-amber-500/10 text-amber-400 border border-amber-500/30 text-[10px] font-mono font-bold uppercase tracking-wider flex items-center gap-1">
            <Clock className="w-3 h-3" />
            <span>Pending Approval</span>
          </span>;
      case "confirmed":
      case "locked":
        return <span className="px-2.5 py-1 rounded-full bg-[#F27D26]/10 text-[#F27D26] border border-[#F27D26]/30 text-[10px] font-mono font-bold uppercase tracking-wider flex items-center gap-1">
            <CheckCircle className="w-3 h-3" />
            <span>Confirmed</span>
          </span>;
      case "pickup_ready":
        return <span className="px-2.5 py-1 rounded-full bg-cyan-500/10 text-cyan-400 border border-cyan-500/30 text-[10px] font-mono font-bold uppercase tracking-wider flex items-center gap-1">
            <Truck className="w-3 h-3" />
            <span>Pickup Ready</span>
          </span>;
      case "active":
        return <span className="px-2.5 py-1 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 text-[10px] font-mono font-bold uppercase tracking-wider flex items-center gap-1">
            <CheckCircle className="w-3 h-3" />
            <span>Active Rental</span>
          </span>;
      case "return_pending":
        return <span className="px-2.5 py-1 rounded-full bg-purple-500/10 text-purple-400 border border-purple-500/30 text-[10px] font-mono font-bold uppercase tracking-wider flex items-center gap-1">
            <Clock className="w-3 h-3" />
            <span>Return Pending</span>
          </span>;
      case "completed":
        return <span className="px-2.5 py-1 rounded-full bg-slate-800 text-slate-300 border border-slate-700 text-[10px] font-mono font-bold uppercase tracking-wider flex items-center gap-1">
            <CheckCircle className="w-3 h-3" />
            <span>Completed</span>
          </span>;
      case "cancelled":
      ...
      default:
        return <span className="px-2.5 py-1 rounded-full bg-[#1A1A1A] text-[#888888] border border-[#333] text-[10px] font-mono font-bold uppercase tracking-wider">
            {status}
          </span>;
    }
  };
  ...
  >
    ...
    className={`px-4 py-2 rounded-xl text-xs font-mono font-bold uppercase tracking-wider transition cursor-pointer flex items-center gap-2 whitespace-nowrap ${activeTab === tab.id ? "bg-[#F27D26] text-black shadow-md" : "bg-[#111111] text-[#888888] hover:text-white hover:bg-[#1A1A1A] border border-[#1F1F1F]"}`}
  >
            <span>{tab.label}</span>
            <span
  ...
  >
    ...
    /* Bookings List */
      ...
      {filteredBookings.length === 0 ? <div className="bg-[#111111] rounded-3xl p-12 text-center border border-[#1F1F1F] max-w-md mx-auto space-y-4 font-mono">
          ...
          <p className="text-xs text-[#888888]">
            {activeTab === "all" ? "You haven't rented any equipment yet. Explore the marketplace to make your first reservation!" : `No bookings currently match the "${activeTab}" status filter.`}
          </p>
          <Link
    to={ROUTES.browse}
  ...
  >
                  ...
                  {(b.status === "pending" || b.status === "confirmed" || b.status === "locked") && onCancelBooking && <button
