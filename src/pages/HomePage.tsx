...
import { Search, MapPin, ShieldCheck, ArrowRight, Zap, Leaf, Truck, CheckCircle2, Wrench } from 'lucide-react';
...
export const HomePage: React.FC<HomePageProps> = ({
  ...
  return (
    <div className="space-y-16 py-8 px-4 sm:px-6 lg:px-8 max-w-7xl mx-auto text-white">
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-2 font-mono scrollbar-none">
          {categories.map((cat) => (
            <button
              ...
              className={`px-4 py-2.5 rounded-2xl text-xs font-bold uppercase tracking-wider whitespace-nowrap transition cursor-pointer border ${
                selectedCategory === cat.name
                  ? 'bg-[#F27D26] text-black border-white shadow-md'
                  : 'bg-[#111111] text-[#888888] border-[#1F1F1F] hover:border-[#333333] hover:text-white'
              }`}
            >
              {cat.name} ({cat.count})
            </button>
