...
export const AuthPage = ({ onLogin, currentUser }) => {
  ...
  const handleSelectDemoUser = async (user) => {
    try {
  ...
  return <div className="max-w-4xl mx-auto px-4 py-8 space-y-8 font-mono text-white">
      {
    /* Title & Header */
  }
  ...
  />
    ...
    className={`p-4 rounded-2xl border text-left transition-all duration-200 cursor-pointer flex flex-col justify-between space-y-3 ${currentUser?.id === account.id ? "bg-[#1A1A1A] border-[#F27D26] shadow-[0_0_15px_rgba(242,125,38,0.2)]" : "bg-[#141414] border-[#222222] hover:border-[#333333] hover:bg-[#1A1A1A]"}`}
  >
    ...
    className={`flex-1 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-2 transition cursor-pointer ${activeTab === "login" ? "bg-[#F27D26] text-black shadow-md" : "text-[#888888] hover:text-white"}`}
  >
    ...
    className={`flex-1 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-2 transition cursor-pointer ${activeTab === "register" ? "bg-[#F27D26] text-black shadow-md" : "text-[#888888] hover:text-white"}`}
  >
        ...
        <form onSubmit={handleSubmit} className="space-y-4 text-xs font-mono">
          {authError && <div className="p-3.5 bg-red-500/10 border border-red-500/30 rounded-xl text-red-400 text-xs flex items-start gap-2 font-mono">
  ...
  >
          ...
          {showPipeline && <div className="mt-4 p-4 rounded-2xl bg-[#0A0A0A] border border-[#222222] space-y-3">
  ...
  >
                {isExecutingPipeline ? <span>Executing Pipeline...</span> : <>
                    <Terminal className="w-3.5 h-3.5" />
                    <span>Run Query Simulation</span>
                  </>}
              </button>
              {pipelineOutput && <pre className="p-3 rounded-xl bg-[#050505] border border-[#222222] text-[10px] text-emerald-300 overflow-x-auto max-h-32 font-mono">
