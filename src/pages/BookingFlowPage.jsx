...
export const BookingFlowPage = ({
  ...
  const initialStart = searchParams.get("start") || (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
  const defaultEnd = searchParams.get("end") || new Date(Date.now() + 864e5 * 3).toISOString().split("T")[0];
  ...
  const diffTime = Math.max(1e3 * 60 * 60 * 24, endObj.getTime() - startObj.getTime());
  const rentalDays = Math.max(1, Math.ceil(diffTime / (1e3 * 60 * 60 * 24)));
  ...
  const handlePaymentSuccess = async () => {
    try {
      await onAddBooking({
        ...
        endDate,
        deliveryMethod,
        deliveryAddress: deliveryMethod === "delivery" ? deliveryAddress : void 0
      });
  ...
  return <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 text-white font-mono">
      {
    /* Back Button */
  }
      <Link
  ...
  />
    ...
    className={`p-4 rounded-2xl border text-left transition cursor-pointer ${deliveryMethod === "delivery" ? "bg-[#1A1A1A] border-[#F27D26] text-white shadow-md" : "bg-[#111111] border-[#222222] text-[#888888] hover:border-[#333333]"}`}
  >
    ...
    className={`p-4 rounded-2xl border text-left transition cursor-pointer ${deliveryMethod === "pickup" ? "bg-[#1A1A1A] border-[#F27D26] text-white shadow-md" : "bg-[#111111] border-[#222222] text-[#888888] hover:border-[#333333]"}`}
  >
            ...
            {deliveryMethod === "delivery" && <div className="space-y-1 pt-2">
  ...
  >
    ...
    /* Stripe Payment Modal */
      ...
      {isStripeModalOpen && <StripePaymentModal
