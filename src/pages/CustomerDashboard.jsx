...
export const CustomerDashboard = ({
  ...
  >
        ...
        {recentBookings.length === 0 ? <p className="text-xs text-[#888888] italic">No active or recent bookings found.</p> : <div className="space-y-3">
            {recentBookings.map((b) => <div
    key={b.id}
  ...
  >
    ...
    /* Saved Favorites Quick Grid */
      ...
      {favoriteItems.length > 0 && <div className="space-y-4 font-mono text-xs">
          <div className="flex items-center justify-between border-b border-[#1F1F1F] pb-3">
            <h2 className="text-lg font-serif italic text-white flex items-center gap-2">
  ...
  >
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {favoriteItems.slice(0, 3).map((item) => <EquipmentCard
    key={item.id}
    equipment={item}
    isFavorite={true}
  ...
  />)}
    ...
    /* Damage Report Modal */
      ...
      {selectedBookingForDamage && <DamageReportModal
