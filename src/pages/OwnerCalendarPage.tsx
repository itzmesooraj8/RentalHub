...
interface OwnerCalendarPageProps {
  currentUser: User | null;
  equipmentList: Equipment[];
  bookings: Booking[];
  onBlockDateRange?: (equipmentId: string, startDate: string, endDate: string, reason: string) => void;
}

export const OwnerCalendarPage: React.FC<OwnerCalendarPageProps> = ({
  currentUser,
  equipmentList,
  bookings,
  onBlockDateRange,
  ...
  const ownerEquipment = currentUser
    ? equipmentList.filter((e) => e.ownerId === currentUser.id || true)
    : equipmentList;
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 text-white font-mono">
      ...
      <div className="bg-[#111111] p-5 rounded-3xl border border-[#1F1F1F] space-y-4 shadow-xl text-xs font-mono">
          ...
          <div className="flex items-center gap-1 bg-[#1A1A1A] p-1 rounded-2xl border border-[#333333]">
            {(['month', 'week', 'day'] as const).map((v) => (
              <button
                ...
                className={`px-3 py-1.5 rounded-xl uppercase font-bold text-[10px] tracking-wider transition ${
                  calendarView === v ? 'bg-[#F27D26] text-black' : 'text-[#888888] hover:text-white'
                }`}
              >
                {v}
              </button>
        ...
        <div className="flex flex-wrap items-center gap-6 pt-3 border-t border-[#1F1F1F] text-[11px] text-[#AAAAAA]">
          ...
          <div className="flex items-center gap-2">
            <span className="w-3 h-3 rounded-full bg-[#F27D26]/20 border border-[#F27D26]"></span>
            ...
            return (
              <div
                ...
                className={`min-h-[90px] p-2 rounded-2xl border transition-all flex flex-col justify-between ${
                  isBooked
                    ? 'bg-[#F27D26]/10 border-[#F27D26]/60 shadow-md'
                    : isMaintenance
                    ? 'bg-amber-500/10 border-amber-500/50'
                    : 'bg-[#1A1A1A] border-[#222222] hover:border-[#333333]'
                }`}
              >
                ...
                <div className="mt-1">
                  {isBooked && (
                    <div className="text-[9px] font-mono text-white font-bold truncate bg-[#F27D26]/20 px-1.5 py-0.5 rounded border border-[#F27D26]/40">
                      Booked #{dayInfo.booking?.id.substring(0, 6)}
                    </div>
                  )}
                  {isMaintenance && (
                    <div className="text-[9px] font-mono text-amber-300 font-bold truncate bg-amber-500/20 px-1.5 py-0.5 rounded border border-amber-500/40">
                      Maintenance Block
                    </div>
                  )}
                </div>
