...
export const AdminDisputesPage = ({
  ...
  const handleResolve = (disputeId, winner) => {
    setDisputes(
      (prev) => prev.map((d) => d.id === disputeId ? { ...d, status: "resolved" } : d)
    );
  ...
  return <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 text-white font-mono">
    ...
    className={`px-4 py-2 rounded-xl text-xs font-mono font-bold uppercase tracking-wider transition ${activeTab === "all" ? "bg-[#F27D26] text-black" : "bg-[#111111] text-[#888888] hover:text-white"}`}
  >
          All Claims ({disputes.length})
        </button>
        <button
    onClick={() => setActiveTab("open")}
    className={`px-4 py-2 rounded-xl text-xs font-mono font-bold uppercase tracking-wider transition ${activeTab === "open" ? "bg-[#F27D26] text-black" : "bg-[#111111] text-[#888888] hover:text-white"}`}
  >
          Open Claims ({disputes.filter((d) => d.status === "open" || d.status === "under_review").length})
        </button>
        <button
    onClick={() => setActiveTab("resolved")}
    className={`px-4 py-2 rounded-xl text-xs font-mono font-bold uppercase tracking-wider transition ${activeTab === "resolved" ? "bg-[#F27D26] text-black" : "bg-[#111111] text-[#888888] hover:text-white"}`}
  >
          Resolved
        </button>
      </div>
    ...
    /* Dispute Cards */
      ...
      {filteredDisputes.length === 0 ? <div className="bg-[#111111] rounded-3xl p-12 text-center border border-[#1F1F1F] max-w-md mx-auto space-y-3 font-mono">
        ...
        </div> : <div className="space-y-6 font-mono text-xs">
          {filteredDisputes.map((d) => <div key={d.id} className="p-6 rounded-3xl bg-[#111111] border border-[#1F1F1F] space-y-5 shadow-xl">
    ...
    /* Photos comparison */
              ...
              {(d.beforePhotos?.length || d.afterPhotos?.length) && <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div className="p-3 bg-[#1A1A1A] rounded-2xl border border-[#262626] space-y-2">
                    ...
                    <div className="flex gap-2 overflow-x-auto">
                      {d.beforePhotos?.map((p, i) => <img key={i} src={p} alt="Before" className="w-20 h-20 rounded-xl object-cover border border-[#333]" />)}
                    </div>
                  </div>
                  <div className="p-3 bg-[#1A1A1A] rounded-2xl border border-[#262626] space-y-2">
                    <span className="text-[10px] font-bold text-[#F27D26] uppercase block">Return Inspection Photos</span>
                    <div className="flex gap-2 overflow-x-auto">
                      {d.afterPhotos?.map((p, i) => <img key={i} src={p} alt="After" className="w-20 h-20 rounded-xl object-cover border border-[#F27D26]/50" />)}
                    </div>
                  </div>
                </div>}
    ...
    /* Resolution Controls */
              ...
              {d.status !== "resolved" ? <div className="flex flex-col sm:flex-row items-center justify-between gap-3 pt-3 border-t border-[#1F1F1F]">
  ...
  >
                ...
                </div> : <div className="p-3 bg-emerald-500/10 border border-emerald-500/30 rounded-2xl text-emerald-400 font-bold uppercase text-[11px]">
