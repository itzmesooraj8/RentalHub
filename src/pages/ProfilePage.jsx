...
export const ProfilePage = ({ user, onUpdateUser, onSwitchRole }) => {
  ...
  />
    ...
    /* Demo Switch Persona Bar (Controlled Demo) */
      ...
      {onSwitchRole && <div className="bg-[#111111] p-4 rounded-2xl border border-[#1F1F1F] flex flex-col sm:flex-row items-center justify-between gap-3 text-xs">
    ...
    className={`px-3 py-1.5 rounded-xl uppercase font-bold text-[10px] tracking-wider transition cursor-pointer border ${user.role === r ? "bg-[#F27D26] text-black border-[#F27D26]" : "bg-[#1A1A1A] text-[#888888] hover:text-white border-[#333333]"}`}
  >
                {r} Mode
              </button>)}
          </div>
  ...
  />
          ...
          <div className="pt-2 flex items-center justify-between">
            {savedSuccess ? <span className="text-emerald-400 font-bold text-xs flex items-center gap-1.5">
