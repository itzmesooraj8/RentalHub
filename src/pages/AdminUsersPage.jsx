...
export const AdminUsersPage = () => {
  ...
  const handleToggleRole = async (userId, currentRole) => {
    const nextRole = currentRole === "customer" ? "owner" : "customer";
    try {
    ...
    /* Header Banner */
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-[#111111] p-6 rounded-3xl border border-[#222222]">
        ...
        <div className="flex items-center gap-3 text-xs font-mono">
          ...
          {pendingCount > 0 && <span className="px-3 py-1.5 rounded-xl bg-amber-500/10 border border-amber-500/20 text-amber-400 font-bold animate-pulse">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left font-mono text-xs">
              ...
              <tbody className="divide-y divide-[#1F1F1F]">
                {filteredUsers.map((u) => <tr key={u.id} className="hover:bg-[#161616] transition">
                    <td className="p-4">
                    ...
                    <td className="p-4">
                      <span
    className={`inline-block px-2.5 py-1 rounded-full text-[10px] font-bold uppercase ${u.role === "admin" ? "bg-rose-500/10 border border-rose-500/20 text-rose-400" : u.role === "owner" ? "bg-[#F27D26]/10 border border-[#F27D26]/20 text-[#F27D26]" : "bg-slate-500/10 border border-slate-500/20 text-slate-300"}`}
  >
                        {u.role}
                      </span>
                    </td>
                    ...
                    <td className="p-4">
                      <span
    className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-[10px] font-bold uppercase ${u.kycStatus === "verified" ? "bg-emerald-500/10 border border-emerald-500/20 text-emerald-400" : u.kycStatus === "pending" ? "bg-amber-500/10 border border-amber-500/20 text-amber-400" : "bg-rose-500/10 border border-rose-500/20 text-rose-400"}`}
  >
