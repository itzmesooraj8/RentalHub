...
export const ComparePage = () => {
  ...
  if (loading) {
    return <div className="min-h-screen bg-[#0A0A0A] text-slate-100 p-8 flex items-center justify-center font-sans">
        <div className="text-center space-y-4">
          <div className="w-10 h-10 border-2 border-[#F27D26] border-t-transparent rounded-full animate-spin mx-auto" />
          <p className="text-xs font-mono text-slate-400">Loading equipment comparison data...</p>
        </div>
      </div>;
  }
  return <div className="min-h-screen bg-[#0A0A0A] text-slate-100 p-6 md:p-12 font-sans space-y-8 max-w-7xl mx-auto">
      {
  ...
  >
      ...
      {items.length === 0 ? <div className="text-center py-20 bg-[#111111] border border-[#222222] rounded-3xl space-y-4">
  ...
  >
    ...
    <div className="overflow-x-auto">
      ...
      return <div
        key={item.id}
        className={`bg-[#111111] border rounded-3xl p-6 space-y-6 flex flex-col justify-between transition relative ${isBestValue ? "border-[#F27D26] shadow-xl shadow-[#F27D26]/10" : "border-[#222222]"}`}
      >
                  {isBestValue && <div className="absolute -top-3 left-6 inline-flex items-center gap-1.5 bg-[#F27D26] text-black px-3 py-0.5 rounded-full text-[10px] font-mono font-bold uppercase tracking-wider shadow-md">
