...
export const NotificationsPage = ({
  ...
  return <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 text-white font-mono">
    ...
    /* Header */
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-[#1F1F1F]">
        ...
        {onMarkAllRead && notifications.some((n) => !n.read) && <button
  ...
  >
    ...
    /* Notifications List */
      ...
      {filteredNotifications.length === 0 ? <div className="bg-[#111111] rounded-3xl p-12 text-center border border-[#1F1F1F] max-w-md mx-auto space-y-3 font-mono">
    ...
    className={`p-4 rounded-2xl border transition-all cursor-pointer flex items-start gap-4 ${!n.read ? "bg-[#181818] border-[#F27D26]/50 shadow-md ring-1 ring-[#F27D26]/20" : "bg-[#111111] border-[#1F1F1F] hover:bg-[#161616]"}`}
  >
              ...
              <div className="flex-1 min-w-0 space-y-1">
                ...
                {n.link && <div className="pt-2 flex items-center gap-1 text-[11px] font-bold text-[#F27D26] hover:underline">
                    <span>View details</span>
                    <ChevronRight className="w-3.5 h-3.5" />
                  </div>}
              </div>

              {!n.read && <span className="w-2 h-2 rounded-full bg-[#F27D26] shrink-0 mt-2" />}
            </div>)}
        </div>}
    </div>;
};
