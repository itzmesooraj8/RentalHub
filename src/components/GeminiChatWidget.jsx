...
export const GeminiChatWidget = () => {
  ...
  >
        ...
        </button> : <div className="w-80 sm:w-96 h-[480px] bg-[#111111] rounded-3xl shadow-2xl border border-[#1F1F1F] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
          {
    /* Header */
  }
  ...
  >
                {m.role === "assistant" && <div className="w-6 h-6 rounded-full bg-[#F27D26] text-black flex items-center justify-center shrink-0 text-[10px] font-bold mt-1">
                    AI
                  </div>}
                <div
    className={`p-3 rounded-2xl max-w-[82%] leading-relaxed ${m.role === "user" ? "bg-[#F27D26] text-black font-medium rounded-tr-none" : "bg-[#111111] text-[#E5E5E5] border border-[#222222] rounded-tl-none shadow-sm"}`}
  >
                  {m.text}
                </div>
              </div>)}
            {loading && <div className="flex items-center gap-2 text-[#888888] text-xs italic font-mono">
