...
export const AiPreDispatchModal = ({
  ...
  const [conditionType, setConditionType] = useState("pickup");
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-md p-4 overflow-y-auto font-mono">
        ...
        <div className="space-y-4">
          <div>
            ...
            <div className="grid grid-cols-3 gap-3">
              {["pickup", "return", "damage"].map((type) => <button
    key={type}
    type="button"
    onClick={() => setConditionType(type)}
    className={`py-2 px-3 rounded-xl text-xs font-bold capitalize border transition-all ${conditionType === type ? "bg-purple-600/20 text-purple-300 border-purple-500/50" : "bg-[#181818] text-slate-400 border-[#262626] hover:border-slate-700"}`}
  >
  ...
  >
            ...
            <div className="flex items-center gap-3 overflow-x-auto pb-2">
              {photos.map((url, idx) => <div key={idx} className="relative w-20 h-20 rounded-xl overflow-hidden border border-[#333] shrink-0">
    ...
    /* Execution Error */
        ...
        {error && <div className="p-3 bg-red-500/10 border border-red-500/30 rounded-xl text-xs text-red-400">
            {error}
          </div>}
    ...
    /* AI Result Card */
        ...
        {result && <div className="bg-[#181818] p-5 rounded-2xl border border-purple-500/30 space-y-4">
            <div className="flex items-center justify-between">
              <span className="text-xs font-bold text-purple-300 flex items-center gap-1.5">
                <FileCheck className="w-4 h-4" /> Gemini 2.5 Flash Structured JSON Audit Log
              </span>
              <span
    className={`text-xs font-bold px-2.5 py-0.5 rounded-full border ${result.recommendedAction === "APPROVE_DISPATCH" ? "bg-emerald-500/10 text-emerald-400 border-emerald-500/30" : "bg-amber-500/10 text-amber-400 border-amber-500/30"}`}
  >
