...
export const Navbar = ({
  ...
  return <header className="sticky top-3 z-50 max-w-7xl mx-auto my-2 px-3 sm:px-4 py-2.5 rounded-2xl md:rounded-full bg-[#0D0D0D]/90 backdrop-blur-xl border border-[#222222] shadow-[0_10px_30px_rgba(0,0,0,0.8)] text-white transition-all duration-300 font-sans">
    ...
    return <Link
      key={link.path}
      to={link.path}
      className={`px-3.5 py-1.5 rounded-full text-xs font-semibold tracking-wide flex items-center gap-1.5 transition-all duration-200 ${active ? "bg-[#F27D26] text-black shadow-[0_0_12px_rgba(242,125,38,0.3)]" : "text-[#888888] hover:text-white hover:bg-[#1C1C1C]"}`}
    >
                <Icon className="w-3.5 h-3.5" />
                <span>{link.label}</span>
                {link.badge !== void 0 && link.badge > 0 && <span
      className={`px-1.5 py-0.2 text-[10px] font-mono rounded-full font-bold ${active ? "bg-black text-white" : "bg-[#F27D26] text-black"}`}
    >
                    {link.badge}
                  </span>}
              </Link>;
  ...
  />
    ...
    /* Dropdown Menu */
              ...
              {isUserMenuOpen && <div className="absolute right-0 mt-2 w-56 bg-[#111111] border border-[#222222] rounded-2xl shadow-2xl p-2 space-y-1 text-xs z-50 animate-in fade-in zoom-in-95 duration-150 font-sans">
  ...
  >
    ...
    /* Demo Role Switcher Gated */
                  ...
                  {switchRole && <div className="pt-2 border-t border-[#222222] space-y-1">
                      ...
                      <div className="grid grid-cols-3 gap-1 px-2 font-mono">
                        {["customer", "owner", "admin"].map((r) => <button
    key={r}
    onClick={() => {
      switchRole(r);
      setIsUserMenuOpen(false);
    }}
    className={`py-1 text-[9px] uppercase font-bold rounded-lg border transition cursor-pointer ${currentUser.role === r ? "bg-[#F27D26] text-black border-[#F27D26]" : "bg-[#1A1A1A] text-[#888888] hover:text-white border-[#333333]"}`}
  >
                            {r}
                          </button>)}
                      </div>
  ...
  >
    ...
    /* Mobile Drawer */
      ...
      {isMobileMenuOpen && <div className="md:hidden mt-3 pt-3 border-t border-[#222222] space-y-1.5 text-xs font-sans">
          {navLinks.map((link) => {
    const Icon = link.icon;
    const active = isLinkActive(link.path);
    return <Link
      ...
      className={`flex items-center justify-between p-3 rounded-xl transition ${active ? "bg-[#F27D26] text-black font-semibold" : "bg-[#141414] text-[#888888]"}`}
    >
                <div className="flex items-center gap-2">
                  <Icon className="w-4 h-4" />
                  <span>{link.label}</span>
                </div>
                {link.badge !== void 0 && link.badge > 0 && <span className="px-2 py-0.5 rounded-full bg-black text-white text-[10px] font-mono">
                    {link.badge}
                  </span>}
              </Link>;
  })}
