...
export const AvailabilityCalendar = ({ equipmentId, onSelectDates, dailyRate }) => {
  ...
  const [blockedDateStrings, setBlockedDateStrings] = useState([]);
  const [loading, setLoading] = useState(false);
  const [selectedStart, setSelectedStart] = useState(15);
  const [selectedEnd, setSelectedEnd] = useState(18);
  useEffect(() => {
    if (!equipmentId) return;
    setLoading(true);
    equipmentService.getAvailability(equipmentId, currentYear, currentMonth).then((dates) => {
  ...
  return <div className="bg-[#111111] rounded-3xl p-6 border border-[#1F1F1F] shadow-xl space-y-4 text-white">
      {
    /* Calendar Header */
  }
    ...
    /* Legend & Summary */
      ...
      <div className="pt-3 border-t border-[#1F1F1F] flex flex-wrap items-center justify-between text-[10px] font-mono text-[#888888] gap-2">
        <div className="flex items-center gap-3">
          <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded bg-[#F27D26]" /> Selected</span>
          <span className="flex items-center gap-1"><span className="w-2.5 h-2.5 rounded bg-[#141414] border border-[#333333]" /> MongoDB Reserved</span>
        </div>

        {calculatedDays > 0 ? <div className="text-right text-xs font-bold text-emerald-400 flex items-center gap-1">
            <CheckCircle className="w-3.5 h-3.5 text-emerald-400" />
            <span>{calculatedDays} Days Selected (₹{estimatedSubtotal} Subtotal)</span>
          </div> : <span className="italic text-[#666666]">Click start and end date to select duration</span>}
      </div>
    </div>;
};
