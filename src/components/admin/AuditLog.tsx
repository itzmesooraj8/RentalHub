...
export interface AuditLogEntry {
  id: string;
  timestamp: string;
  actorRole: 'customer' | 'owner' | 'admin' | 'system' | string;
  actorName: string;
  action: string;
  targetId: string;
  metadata?: string;
...
export const AuditLog: React.FC<AuditLogProps> = ({ logs: propLogs }) => {
  const [fetchedLogs, setFetchedLogs] = useState<AuditLogItem[]>([]);
  const [loading, setLoading] = useState<boolean>(!propLogs);
  ...
  return (
    <div className="bg-[#111111] rounded-3xl p-6 border border-[#1F1F1F] shadow-xl space-y-4 font-sans text-xs">
      ...
      {loading ? (
        <div className="p-8 text-center text-slate-400 font-mono">Loading MongoDB audit log stream...</div>
      ) : logs.length === 0 ? (
        <div className="p-8 text-center text-slate-500 font-mono">No security audit records logged yet.</div>
      ) : (
