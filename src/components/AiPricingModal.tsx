...
interface AiPricingModalProps {
  equipment: Equipment;
  onClose: () => void;
  onUpdateRate: (equipmentId: string, newRate: number) => void;
}
...
export const AiPricingModal: React.FC<AiPricingModalProps> = ({
  ...
  const [activeTab, setActiveTab] = useState<'trends' | 'pipeline'>('trends');
  ...
  // Simulated Weekly Demand Trend Data (from MongoDB Aggregation Pipeline)
  const weeklyTrends = [
  ...
  const handleGeneratePriceAdvice = async () => {
    ...
    try {
      ...
      if (!apiKey) {
        // Fallback realistic AI recommendation if key is not configured locally
        setTimeout(() => {
          setAiRecommendation({
      ...
      <div className="bg-[#111111] rounded-3xl max-w-lg w-full p-6 shadow-2xl border border-[#1F1F1F] text-white relative space-y-4 font-mono max-h-[92vh] overflow-y-auto">
        ...
        <div className="flex items-center gap-2 bg-[#1A1A1A] p-1 rounded-2xl border border-[#222222]">
          <button
            ...
            className={`flex-1 py-1.5 rounded-xl text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-1.5 transition cursor-pointer ${
              activeTab === 'trends' ? 'bg-[#F27D26] text-black shadow-md' : 'text-[#888888] hover:text-white'
            }`}
          >
          ...
          <button
            ...
            className={`flex-1 py-1.5 rounded-xl text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-1.5 transition cursor-pointer ${
              activeTab === 'pipeline' ? 'bg-[#F27D26] text-black shadow-md' : 'text-[#888888] hover:text-white'
            }`}
          >
        ...
        {activeTab === 'trends' ? (
          <div className="space-y-4">
            ...
            <div className="bg-[#1A1A1A] rounded-2xl p-4 border border-[#222222] space-y-3">
              ...
              <div className="space-y-2">
                {weeklyTrends.map((wt, i) => (
                  <div key={i} className="flex items-center justify-between text-xs p-2 rounded-xl bg-[#111111] border border-[#2A2A2A]">
                    <span className="text-[#888888] w-28">{wt.week}</span>
                    <div className="flex-1 mx-3 bg-[#222222] h-2 rounded-full overflow-hidden">
                      <div
                        className={`h-full ${
                          wt.status.includes('Peak') ? 'bg-[#F27D26]' : wt.status.includes('High') ? 'bg-amber-400' : 'bg-emerald-500'
                        }`}
                        style={{ width: wt.utilization }}
                      ></div>
                    </div>
                    <span className="font-bold text-white w-12 text-right">{wt.utilization}</span>
                    {wt.status.includes('Peak') && (
                      <span className="ml-2 px-1.5 py-0.5 bg-[#F27D26]/20 text-[#F27D26] border border-[#F27D26]/40 rounded text-[9px] font-bold flex items-center gap-1">
            ...
            {!aiRecommendation ? (
              ...
              <div className="p-4 rounded-2xl bg-[#1A1A1A] border border-[#333333] space-y-3">
                <div className="flex items-center justify-between">
