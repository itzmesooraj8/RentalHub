...
import { EscrowLedger, User } from '../types';

interface EscrowLedgerCardProps {
  bookingId: string;
  currentUser: User | null;
  onLedgerUpdate?: (updated: EscrowLedger) => void;
}

export const EscrowLedgerCard: React.FC<EscrowLedgerCardProps> = ({
  bookingId,
  currentUser,
  onLedgerUpdate,
}) => {
  ...
  if (loading) {
    return (
      <div className="bg-[#121212] p-6 rounded-2xl border border-[#262626] animate-pulse flex items-center justify-center text-slate-400 gap-3">
  ...
  return (
    <div className="bg-[#121212] rounded-3xl border border-[#262626] p-6 shadow-xl space-y-5 text-white font-mono">
      {/* Header */}
      ...
      <div className="space-y-3">
        ...
        <div className="space-y-2 max-h-48 overflow-y-auto pr-1">
          {ledger.ledgerHistory.map((item, idx) => (
            <div key={idx} className="bg-[#181818] p-3 rounded-xl border border-[#262626] text-xs space-y-1">
