...
import { Camera, X, Check, Upload, AlertTriangle, ShieldAlert, Image as ImageIcon } from "lucide-react";
export const DamageReportModal = ({
  booking,
  onClose,
  onSubmitConditionReport
}) => {
  const [activePhotoTab, setActivePhotoTab] = useState("after");
  ...
  const handleAddQuickSample = (url) => {
    if (activePhotoTab === "before") {
  ...
  return <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 font-mono">
  ...
  >
    ...
    className={`flex-1 py-2 rounded-xl text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-1.5 transition cursor-pointer ${activePhotoTab === "before" ? "bg-[#F27D26] text-black shadow-md" : "text-[#888888] hover:text-white"}`}
  >
    ...
    className={`flex-1 py-2 rounded-xl text-xs font-bold uppercase tracking-wider flex items-center justify-center gap-1.5 transition cursor-pointer ${activePhotoTab === "after" ? "bg-[#F27D26] text-black shadow-md" : "text-[#888888] hover:text-white"}`}
  >
            <ImageIcon className="w-3.5 h-3.5" />
  ...
  >
          ...
          <div className="grid grid-cols-3 gap-3">
            {(activePhotoTab === "before" ? beforePhotos : afterPhotos).map((photoUrl, idx) => <div key={idx} className="relative aspect-square rounded-2xl overflow-hidden border border-[#333333] bg-[#1A1A1A]">
  ...
  />
          ...
          {conditionNotes.trim().length > 0 && <div className="p-3 rounded-xl bg-[#F27D26]/10 border border-[#F27D26]/30 text-[11px] text-[#F27D26] flex items-center gap-2">
