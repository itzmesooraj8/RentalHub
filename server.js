...
import { authenticate, requireRole, generateToken } from "./server/middleware/auth.js";
import { validateBody, registerSchema, loginSchema, equipmentSchema, bookingSchema } from "./server/middleware/validation.js";
import { generateDynamicPricing, generateRentalAiAssistantResponse, analyzePreDispatchCondition, evaluateBookingRiskScore } from "./server/geminiService.js";
const sseClients = [];
...
async function startServer() {
  ...
  app.use(
    helmet({
      ...
      // Disabled for Vite inline development HMR & asset loading
    })
  );
  app.use(cors());
...
`);
  ...
  app.post("/api/auth/register", validateBody(registerSchema), async (req, res) => {
    ...
    const newUser = {
      ...
      memberSince: (/* @__PURE__ */ new Date()).toLocaleDateString("en-US", { month: "long", year: "numeric" }),
  ...
  app.post("/api/auth/login", validateBody(loginSchema), async (req, res) => {
    ...
    sendSuccess(res, { token, user: userDoc });
  });
  ...
  app.post("/api/auth/favorite", authenticate, async (req, res) => {
  ...
  app.get("/api/equipment", async (req, res) => {
    ...
    const items = await db.getEquipment({
      ...
      maxPrice: maxPrice ? Number(maxPrice) : void 0,
      location,
      lat: lat ? Number(lat) : void 0,
      lng: lng ? Number(lng) : void 0,
      startDate,
      endDate,
      onlyAvailable: onlyAvailable === "true",
      sort,
  ...
  app.post("/api/equipment", authenticate, requireRole("owner", "admin"), validateBody(equipmentSchema), async (req, res) => {
  ...
  app.put("/api/equipment/:id", authenticate, requireRole("owner", "admin"), async (req, res) => {
  ...
  app.delete("/api/equipment/:id", authenticate, requireRole("owner", "admin"), async (req, res) => {
    const existing = await db.getEquipmentById(req.params.id);
    if (!existing) return sendError(res, "NOT_FOUND", "Equipment listing not found.", 404);
    if (existing.ownerId !== req.user.id && req.user.role !== "admin") {
      return sendError(res, "FORBIDDEN", "Access denied. You do not own this equipment listing.", 403);
  ...
  app.post("/api/bookings", authenticate, validateBody(bookingSchema), async (req, res) => {
    ...
    const rentalDays = Math.max(1, Math.ceil((end.getTime() - start.getTime()) / (1e3 * 3600 * 24)));
  ...
  app.post("/api/bookings/:id/condition", authenticate, async (req, res) => {
  ...
  app.get("/api/notifications", authenticate, async (req, res) => {
    const notifs = await db.getNotifications(req.user.role === "admin" ? void 0 : req.user.id);
  ...
  app.patch("/api/notifications/read-all", authenticate, async (req, res) => {
    const updated = await db.markAllNotificationsRead(req.user.role === "admin" ? void 0 : req.user.id);
    sendSuccess(res, { success: updated });
  });
  app.get("/api/payments/escrow-ledger/:bookingId", authenticate, async (req, res) => {
  ...
  app.post("/api/payments/escrow-hold", authenticate, async (req, res) => {
  ...
  app.post("/api/payments/escrow-release", authenticate, async (req, res) => {
    const { bookingId } = req.body;
    if (!bookingId) return sendError(res, "VALIDATION_ERROR", "bookingId is required.");
    const booking = await db.getBookingById(bookingId);
    if (!booking) return sendError(res, "NOT_FOUND", "Booking record not found.", 404);
  ...
  app.post("/api/payments/escrow-dispute", authenticate, async (req, res) => {
  ...
  app.post("/api/ai/pre-dispatch-inspection", authenticate, async (req, res) => {
    ...
    const booking = await db.getBookingById(bookingId);
    if (!booking) return sendError(res, "NOT_FOUND", "Booking record not found.", 404);
    const type = conditionType || "pickup";
  ...
  app.post("/api/ai/booking-risk-score", authenticate, async (req, res) => {
    ...
    const rentalDays = Math.max(1, Math.ceil((eDate.getTime() - sDate.getTime()) / (1e3 * 3600 * 24)));
  ...
  app.post("/api/ai/smart-search", async (req, res) => {
    ...
    const allEquipment = await db.getEquipment({ search: query, maxPrice: maxPrice ? Number(maxPrice) : void 0 });
    const items = Array.isArray(allEquipment) ? allEquipment : allEquipment.items || [];
    const rankedResults = items.map((item, idx) => {
  ...
  app.get("/api/reviews", async (req, res) => {
    const reviews = await db.getReviews(req.query.equipmentId);
  ...
  app.get("/api/disputes", authenticate, requireRole("admin"), async (req, res) => {
    const disputes = await db.getDisputes();
    sendSuccess(res, disputes);
  });
  app.post("/api/disputes/:id/resolve", authenticate, requireRole("admin"), async (req, res) => {
  ...
  app.get("/api/analytics/owner/:ownerId", authenticate, requireRole("owner", "admin"), async (req, res) => {
    const analytics = await db.getOwnerAnalytics(req.params.ownerId);
    sendSuccess(res, analytics);
  });
  app.get("/api/analytics/admin", authenticate, async (req, res) => {
  ...
  app.post("/api/webhooks/stripe", async (req, res) => {
    ...
    console.log(`\u{1F4B3} [Stripe Webhook Router] Processing event ${type} for booking ${bookingId}`);
  ...
  app.post("/api/ai/recommend-pricing", authenticate, requireRole("owner", "admin"), async (req, res) => {
    const { equipmentId } = req.body;
    const equipment = await db.getEquipmentById(equipmentId);
    if (!equipment) return sendError(res, "NOT_FOUND", "Equipment not found.", 404);
